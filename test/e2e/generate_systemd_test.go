...
var _ = Describe("Podman generate systemd", func() {
	...
	It("podman generate systemd pod with --no-header option", func() {
		tmpDir, err := ioutil.TempDir("", "")
		Expect(err).To(BeNil())
		tmpFile := tmpDir + "podID"
		defer os.RemoveAll(tmpDir)
		...
		session := podmanTest.Podman([]string{"generate", "systemd", "--new", "--name", "foo", "--no-header"})
		session.WaitWithDefaultTimeout()
		Expect(session.ExitCode()).To(Equal(0))
		...
		// Grepping the output (in addition to unit tests)
		Expect(session.OutputToString()).To(ContainSubstring("# pod-foo.service"))
		Expect(session.OutputToString()).NotTo(ContainSubstring("# autogenerated by Podman"))
	})
